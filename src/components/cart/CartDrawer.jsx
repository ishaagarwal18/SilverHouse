...
import { 
  ...
  Scale, Award, Lock, ChevronRight, Heart, Eye
} from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end font-sans">
      ...
      <div className="relative w-full max-w-lg bg-[var(--th-card)] text-[var(--th-text-main)] h-full shadow-2xl flex flex-col justify-between z-10 animate-in slide-in-from-right duration-300 border-l border-[var(--th-border)] transition-colors">
        ...
        <div className="px-5 py-3 bg-gradient-to-r from-[var(--th-surface-alt)] via-[var(--th-card)] to-[var(--th-surface-alt)] border-b border-[var(--th-border)] shrink-0">
          ...
          {/* Meter Track */}
          <div className="w-full bg-[var(--th-border)]/70 rounded-full h-1.5 overflow-hidden p-0.5">
            <div 
        ...
        {/* 3. Spacious Cart Content Scrollable Area */}
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
          {cartItems.length === 0 ? (
            ...
            <>
                ...
                {cartItems.map((item, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className="p-3.5 sm:p-4 rounded-2xl border border-[var(--th-border)] bg-[var(--th-card)] shadow-xs hover:shadow-md hover:border-[var(--th-primary)]/40 transition-all relative group"
                    >
                      <div className="flex items-start space-x-3.5 sm:space-x-4">
                        
                        {/* Interactive Thumbnail - Click opens Quick View */}
                        <div 
                          onClick={() => onQuickView && onQuickView(item.product)}
                          className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-2xl overflow-hidden bg-[var(--th-surface-alt)] border border-[var(--th-border)] shrink-0 shadow-xs cursor-pointer group/thumb"
                        ...
                        >
                          ...
                          <span className="absolute bottom-1 left-1 px-1.5 py-0.5 rounded-md text-[8px] sm:text-[9px] font-extrabold uppercase tracking-wider bg-black/75 text-white backdrop-blur-xs font-mono">
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-between self-stretch">
                          <div>
                            <div className="flex justify-between items-start gap-2">
                              <div>
                                <h4 
                                  onClick={() => onQuickView && onQuickView(item.product)}
                                  className="font-serif font-bold text-sm sm:text-base text-[var(--th-text-main)] hover:text-[var(--th-primary)] cursor-pointer leading-snug line-clamp-1 transition-colors"
                                  title="Click for Quick View"
                                >
                                ...
                                <button
                                  type="button"
                                  onClick={() => onQuickView && onQuickView(item.product)}
                                  className="inline-flex items-center space-x-1 text-[11px] font-semibold text-[var(--th-primary)] hover:underline mt-0.5 cursor-pointer"
                                >
                          ...
                          <div className="flex items-center justify-between pt-2.5 mt-2 border-t border-[var(--th-border)]/70">
                            <div>
                            ...
                            {/* Tactile Stepper */}
                            <div className="flex items-center space-x-1.5 bg-[var(--th-surface-alt)] border border-[var(--th-border)] rounded-xl p-1 shadow-2xs">
                              <button
              ...
              {/* Promo Code & Voucher Section (Inside Scrollable Area) */}
              <div className="pt-2">
                {appliedCoupon ? (
                  ...
                  <div className="space-y-2 p-3.5 rounded-2xl bg-[var(--th-surface-alt)] border border-[var(--th-border)]">
                    <form onSubmit={(e) => { e.preventDefault(); handleApplyCoupon(); }} className="flex space-x-2">
                      <div className="relative flex-1">
                        <Tag className="w-4 h-4 text-[var(--th-text-muted)] absolute left-3 top-1/2 -translate-y-1/2" />
                        <input
                          type="text"
                          value={couponInput}
                          onChange={(e) => setCouponInput(e.target.value)}
                          placeholder="Enter Promo Code (e.g. SACRED10)"
                          className="w-full bg-[var(--th-card)] border border-[var(--th-border)] focus:border-[var(--th-primary)] text-xs rounded-xl pl-9 pr-3 py-2 text-[var(--th-text-main)] uppercase tracking-wider outline-none placeholder:normal-case placeholder:text-[var(--th-text-muted)]/60 shadow-2xs transition-colors"
                        />
                      </div>
                      <button
                        type="submit"
                        className="px-4 py-2 bg-[var(--th-primary)] hover:bg-[var(--th-primary-hover)] text-white text-xs font-bold rounded-xl uppercase tracking-wider transition-all cursor-pointer shadow-sm hover:shadow-md"
                      >
                        Apply
                      </button>
                    </form>
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                      ...
                      <button
                        ...
                        className="px-2 py-0.5 rounded-lg bg-[var(--th-card)] hover:bg-[var(--th-primary)]/10 text-[var(--th-primary)] border border-[var(--th-border)] text-[10px] font-bold font-mono transition-colors cursor-pointer"
                      >
                        SACRED10 (10% OFF)
                      </button>
                      <button
              ...
              <div className="p-3.5 sm:p-4 rounded-2xl bg-[var(--th-surface-alt)] border border-[var(--th-border)] space-y-2 text-xs text-[var(--th-text-muted)] shadow-2xs">
              ...
              <div className="grid grid-cols-3 gap-2 text-center text-[10px] text-[var(--th-text-muted)] pt-1">
                <div className="p-2 rounded-xl bg-[var(--th-surface-alt)] border border-[var(--th-border)] flex flex-col items-center justify-center">
                  <Award className="w-3.5 h-3.5 text-[var(--th-accent)] mb-0.5" />
                  <span className="font-bold text-[var(--th-text-main)]">BIS 925</span>
                  <span>Hallmarked</span>
                </div>
                <div className="p-2 rounded-xl bg-[var(--th-surface-alt)] border border-[var(--th-border)] flex flex-col items-center justify-center">
                  <Truck className="w-3.5 h-3.5 text-[var(--th-accent)] mb-0.5" />
                  <span className="font-bold text-[var(--th-text-main)]">100% Insured</span>
                  <span>Free Transit</span>
                </div>
                <div className="p-2 rounded-xl bg-[var(--th-surface-alt)] border border-[var(--th-border)] flex flex-col items-center justify-center">
                  <RefreshCw className="w-3.5 h-3.5 text-[var(--th-accent)] mb-0.5" />
        ...
        {/* 4. Compact Sticky Bottom Action Bar */}
        {cartItems.length > 0 && (
          <div className="p-3.5 sm:p-4 bg-[var(--th-card)] border-t border-[var(--th-border)] shadow-lg shrink-0 space-y-2.5">
            <div className="flex items-center justify-between px-1">
              <div>
                <div className="flex items-baseline space-x-1.5">
                  <span className="text-xs text-[var(--th-text-muted)] font-medium">Total Payable:</span>
                  <span className="font-outfit font-extrabold text-lg sm:text-xl text-[var(--th-primary)]">
                    ₹{finalTotal.toLocaleString('en-IN')}
                  </span>
                </div>
                <span className="text-[10px] text-[var(--th-text-muted)] block">
                  Inclusive of 3% GST & Insured Delivery
                </span>
              </div>

              {discountAmount > 0 && (
                <span className="text-[11px] font-bold px-2 py-0.5 rounded-md bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300">
            ...
            <button
              ...
              className="w-full py-3 px-4 sm:py-3.5 sm:px-5 rounded-xl bg-gradient-to-r from-[var(--th-primary)] to-[var(--th-primary-hover)] text-white font-bold text-xs uppercase tracking-widest shadow-md hover:shadow-lg transition-all flex items-center justify-between cursor-pointer group hover:scale-[1.005]"
            >
              <div className="text-left flex items-center space-x-2">
                <span className="font-sans text-xs font-extrabold tracking-wider">
                  Proceed to Checkout
                </span>
                <span className="hidden sm:inline-block text-[10px] text-white/80 font-normal border-l border-white/30 pl-2">
                  100% Insured
                </span>
              </div>
              <div className="flex items-center space-x-2 bg-white/20 px-3 py-1 rounded-lg backdrop-blur-xs">
                <span className="font-outfit font-black text-sm">₹{finalTotal.toLocaleString('en-IN')}</span>
